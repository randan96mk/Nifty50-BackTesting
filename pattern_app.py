...
=============================================
Streamlit UI for detecting and backtesting breakout patterns on daily
Nifty 50 OHLC data.  No volume data required.
...
# ── Page config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Nifty 50 Pattern Backtester",
    page_icon="📈",
    layout="wide",
)

st.title("Nifty 50 — Chart Pattern Breakout Backtester")
st.caption("Detects Triple Tops/Bottoms, Triangles, and Flags/Rectangles on daily OHLC data")
...
if data_source == "Upload CSV File":
    ...
    st.sidebar.markdown("Generates synthetic daily Nifty 50 data (2015–2024).")
    col_s, col_e = st.sidebar.columns(2)
    sample_start = col_s.date_input("Start", value=pd.to_datetime("2015-01-01"))
    ...
    if "daily_df" in st.session_state:
        ...
        st.info("Click **Generate Data** to create sample Nifty 50 data.")
        st.stop()
...
with st.sidebar.expander("Triple Top / Bottom", expanded=False):
    tt_peak_order = st.slider("Peak/Trough sensitivity", 5, 20, 10, 1,
                              key="tt_order",
                              help="Higher = fewer, more significant peaks")
    tt_tolerance = st.slider("Price tolerance (%)", 0.5, 4.0, 1.5, 0.5,
                             key="tt_tol")
    tt_min_spacing = st.slider("Min spacing (bars)", 5, 30, 15, 5,
                               key="tt_sp")
    tt_max_pattern = st.slider("Max pattern span (bars)", 60, 200, 120, 10,
                               key="tt_max")

with st.sidebar.expander("Triangles", expanded=False):
    tri_peak_order = st.slider("Peak/Trough sensitivity", 3, 15, 5, 1,
                               key="tri_order")
    tri_min_window = st.slider("Min window (bars)", 15, 40, 25, 5,
                               key="tri_min")
    tri_max_window = st.slider("Max window (bars)", 50, 120, 80, 10,
                               key="tri_max")

with st.sidebar.expander("Flags / Rectangles", expanded=False):
    fl_pole_lookback = st.slider("Pole lookback (bars)", 5, 30, 15, 5,
                                 key="fl_pole")
    fl_pole_min = st.slider("Min pole move (%)", 2.0, 8.0, 4.0, 0.5,
                            key="fl_pmin")
    fl_consol_min = st.slider("Min consolidation (bars)", 5, 15, 8, 1,
                              key="fl_cmin")
    fl_consol_max = st.slider("Max consolidation (bars)", 15, 50, 30, 5,
                              key="fl_cmax")

max_hold = st.sidebar.slider("Max hold period (days)", 10, 60, 30, 5)
...
for _, trade in trades_df.iterrows():
    ...
    fig_main.add_trace(go.Scatter(
        ...
        hovertext=(
            f"{trade['pattern']}<br>"
            f"Entry: {trade['entry_price']:.1f}<br>"
...
end_idx = min(len(chart_df) - 1, b_idx + int(trade["hold_days"]) + 15)
...
if ptype in ("Triple Top", "Triple Bottom"):
    # Draw horizontal lines at resistance/support and neckline
    if ptype == "Triple Top":
        res_level = trade.get("resistance", None)
        neck_level = trade.get("neckline", None)
        p_indices = [trade.get(f"peak{i}_idx") for i in range(1, 4)]
        if res_level:
    ...
    if u_slope and u_int and l_slope and l_int:
        ws, we = int(ws), int(we)
        line_x = [chart_df.iloc[ws]["date"], chart_df.iloc[min(we, len(chart_df) - 1)]["date"]]
    ...
    if c_high and c_low and cs_idx is not None and ce_idx is not None:
        ...
        cs_date = chart_df.iloc[min(cs_idx, len(chart_df) - 1)]["date"]
        ce_date = chart_df.iloc[min(ce_idx, len(chart_df) - 1)]["date"]
        fig_detail.add_shape(
            type="rect", x0=cs_date, x1=ce_date,
            y0=c_low, y1=c_high,
            fillcolor="rgba(243, 156, 18, 0.15)",
            line=dict(color="#f39c12", dash="dash", width=1))
        fig_detail.add_annotation(
            x=cs_date, y=c_high, text=f"Range: {c_high:.0f}–{c_low:.0f}",
...
fig_detail.add_annotation(
    ...
    text=(
        ...
        f"{trade['exit_reason']} | Hold: {trade['hold_days']}d"
    ),
...
# Sort trades by entry date for proper equity curve
sorted_trades = trades_df.sort_values("entry_date").reset_index(drop=True)
equity = sorted_trades["pnl_points"].cumsum()

fig_eq = go.Figure()
fig_eq.add_trace(go.Scatter(
    x=sorted_trades["entry_date"], y=equity,
    mode="lines+markers",
...
fig_eq.update_layout(
    xaxis_title="Date", yaxis_title="Cumulative P&L (points)",
...
with col_bar:
    ...
    for ptype in sorted_trades["pattern"].unique():
        ...
        fig_bar.add_trace(go.Bar(
            x=sub["entry_date"], y=sub["pnl_points"],
            name=ptype,
            marker_color=color_map.get(ptype, "#888"),
        ))
    fig_bar.update_layout(
        barmode="group", height=380,
        xaxis_title="Entry Date", yaxis_title="P&L (points)",
...
fig_dd.add_trace(go.Scatter(
    x=sorted_trades["entry_date"], y=dd,
    fill="tozeroy", mode="lines",
    line=dict(color="#e74c3c", width=1),
    fillcolor="rgba(231,76,60,0.2)", name="Drawdown",
))
fig_dd.update_layout(
    xaxis_title="Date", yaxis_title="Drawdown (points)",
...
display_cols = ["pattern", "direction", "entry_date", "entry_price",
                "sl", "target", "exit_price", "exit_date", "exit_reason",
                "hold_days", "pnl_points", "pnl_pct", "actual_rr"]
